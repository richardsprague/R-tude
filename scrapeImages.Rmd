---
title: "Scrape Images from a Website"
...
A roofing company sent me their estimate, plus a pageful of photos they took of my roof. I'd like permanent copies of the photos, but rather than download each one individually, can I scrape the html page and save them automatically?
...
The `rvest` package, which installs automatically as part of the Tidyverse, lets me read that page like this:
```{r, message = FALSE}
library(tidyverse)
...
Now I have a bunch of photo urls, so I read them using the [`magick`](https://docs.ropensci.org/magick/articles/intro.html) package.
...
Reading each photo can take some time, so in this example I only bother reading the first one, the element `photos_urls[1]`.
